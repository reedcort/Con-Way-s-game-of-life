...
def generate_world(opts):
    ...
    ## TASK 1 #############################################################
    if opts.world_type == "empty": # checks the world type
        for row in range(opts.rows): # counts number of rows
            temp=[]                 # creates temporary matrix
            for col in range(opts.cols): # counts number of columns
                temp.append(0)      # fills corresponding columns and rows with zeros
            world.append(temp)      # the empty world matrix replaces the temporary matrix
        return world

    if opts.world_type == "random": # checks the world type
        for row in range(opts.rows): # counts number of rows
            temp=[]                 # creates temporary matrix
            for col in range(opts.cols): #counts number of columns
                temp.append(0)      # fills corresponding columns and rows with zeros
            world.append(temp)      # the empty world matrix replaces the temporary matrix
        for new_row in range(0, opts.rows):
            for new_col in range(0, opts.cols):
                # makes 10% of the world living cells
                if rand() <= .10:
                    world.append(1)
        return world
    #######################################################################
...
def update_frame(frame_num, opts, world, img):
    ...
    ## TASK 3 #############################################################
    R =opts.rows
    for x in range(len(new_world)):
        for y in range(len(new_world[0])):
            # finds the total of surrounding neighbors
            # boundaries wrap around
            total = (world[(x - 1) % R][(y - 1) % R] + world[(x - 1) % R][y % R]+
                        world[(x - 1) % R][(y + 1) % R] + world[x % R][(y - 1) % R] +
                        world[x % R][(y + 1) % R] + world[(x + 1) % R][(y - 1) % R] +
                        world[(x + 1) % R][y % R]+ world[(x + 1) % R][(y + 1) % R])

            # Conway's rule for determining if a surrounding cell lives or dies
            if world[x][y] == 1:
...
def blit(world, sprite, x, y):
    ...
    """
    ## TASK 2 #############################################################
    for rows in range(len(sprite)): # number of rows in sprite
        for cols in range(len(sprite[0])): # number of columns
            world[rows + y][cols + x] = sprite[rows][cols] # Adds sprite pixel pattern into world
    #######################################################################
